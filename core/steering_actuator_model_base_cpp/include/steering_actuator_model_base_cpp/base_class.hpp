...
#include "steering_actuator_model_base_cpp/conditional_auxiliary_input.hpp"
...
class SteeringActuatorModelBase
// Derive from a helper struct that add a function only if AuxiliaryInputT is not void
: public auxiliary::conditional_compilation::AuxiliaryInputHelperSA<AuxiliaryInputT>
...
public:
  SteeringActuatorModelBase() = default;
  virtual ~SteeringActuatorModelBase() = default;
  ...
  virtual void set_load(
    const tam::types::common::DataPerWheel<double> & steering_load_torque_per_wheel_Nm) = 0;

  // Model step function
  virtual void evaluate() = 0;

  // getters
  virtual tam::types::common::DataPerWheel<double> get_steering_angles() const = 0;
